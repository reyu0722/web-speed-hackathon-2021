      ...
      <div className="grid gap-1 grid-cols-2 grid-rows-2 w-full h-full border border-gray-300 rounded-lg overflow-hidden">
        {images.map((image, idx) => {
          return (
            ...
            >
              <CoveredImage alt={image.alt} src={getImagePath(image.id)} horizontal={images.length === 1 || images.length === 4 || (images.length === 3 && idx !== 0)} lazy={lazy} />
            </div>
          );
        })}
      </div>
